...
export default function BottomNav({ screen, lang, onFeed, onFavs, onAdd, onMessages, onProfile, unreadMessages = 0 }) {
  return (
    <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md flex justify-around items-end py-2 pb-[max(8px,env(safe-area-inset-bottom))] bg-white/95 backdrop-blur border-t border-gray-100 z-50">
      <button
        className={`flex flex-col items-center gap-0.5 text-[10px] font-semibold px-2 py-1 ${screen === 'feed' ? 'text-brand' : 'text-gray-400'}`}
        onClick={onFeed}
      >
      ...
      <button
        ...
        onClick={onAdd}
      >
        <div className="w-12 h-12 rounded-2xl gradient-btn flex items-center justify-center -mt-6 shadow-lg shadow-brand/20">
      ...
      <button
        className={`relative flex flex-col items-center gap-0.5 text-[10px] font-semibold px-2 py-1 ${screen === 'messages' ? 'text-brand' : 'text-gray-400'}`}
        onClick={onMessages}
      >
        <div className="relative">
          ...
          {unreadMessages > 0 && (
            <span className="absolute -top-1 -right-1 min-w-[16px] h-4 px-0.5 bg-brand text-white text-[9px] font-bold rounded-full flex items-center justify-center leading-none">
      ...
      <button
        className={`flex flex-col items-center gap-0.5 text-[10px] font-semibold px-2 py-1 ${screen === 'profile' ? 'text-brand' : 'text-gray-400'}`}
        onClick={onProfile}
      >
