...
│   ├── BottomNav.jsx           # Нижняя навигация: Лента, Избранное, +, Сообщения, Профиль
...
│   ├── SettingsView.jsx        # Настройки: язык, приватность сообщений
│   ├── MessagesScreen.jsx      # Список диалогов
│   ├── ChatScreen.jsx          # Чат с realtime подпиской (Supabase postgres_changes)
│   ├── RecipeExportView.jsx    # Экспорт рецепта в PDF
│   └── OnboardingView.jsx      # Обязательная настройка профиля при первом входе
└── lib/
...
- `user_settings` – user_id (PK), ui_lang, recipe_lang, message_privacy
...
- `creator` – может всё
- `admin` – может всё, может менять роли пользователей
- `cook` – может добавлять рецепты (дефолт при регистрации)
- `premium` – может добавлять рецепты
- `guest` – только просмотр, лайки, избранное, комментарии. Кнопка "+" показывает заглушку.
...
- Шрифты: Playfair Display (заголовки, `.font-display`), Inter (body/UI)
- Цвета: brand (#E94E1B томатный), danger (#DC2626), bg (#FFFFFF), surface (#F8F5F1)
- Без TypeScript (пока)
...
Стиль: clean iOS + food editorial (Instagram x Pinterest x gastro journal).

- Мобильный-first, max-width 480px
- Белый фон (#FFFFFF), поверхности (#F8F5F1), бордеры (#EAEAEA)
- Текст: основной #1C1C1C, вторичный #6B6B6B
- Основной цвет: #E94E1B (томатный) – кнопки, акценты, иконки активного состояния
- Типографика: Playfair Display для заголовков, Inter для всего UI
- Карточки: border-radius 20px, тень 0 8px 24px rgba(0,0,0,0.06)
- Без градиентов (`.gradient-btn` и `.gradient-text` – просто solid #E94E1B)
- Нижняя навигация с плавающей кнопкой "+"
- Бейдж непрочитанных сообщений на иконке сообщений в BottomNav
